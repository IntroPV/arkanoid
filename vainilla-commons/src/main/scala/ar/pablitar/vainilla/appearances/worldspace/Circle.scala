...
case class Rectangle(val width: Double, val height: Double, color: Color)(implicit val camera: Camera) extends WorldSpaceAppearance {
  def doRenderAt(x: Double, y: Double, graphics: Graphics2D): Unit = {
    graphics.setColor(this.color);
    graphics.fillRect(x.toInt, y.toInt, this.height.toInt, this.width.toInt);
  }
